...
const KPI_CONFIG = [
  ...
  { key: 'proximosVencer',label: 'Próx. Vencer',   color: 'text-orange-300',bg: 'bg-orange-500/10' },
]

const PRIORITY_COLORS = { ALTA: '#f97316', MEDIA: '#eab308', BAJA: '#3b82f6' }
const STATUS_COLORS   = { Abierto: '#22c55e', Cerrado: '#64748b', Vencido: '#ef4444' }
...
export default function DashboardPage() {
  ...
  const byStatus = [
    { name: 'Abierto', value: stats.abiertos },
    { name: 'Vencido', value: stats.vencidos },
    { name: 'Cerrado', value: stats.cerrados },
  ].filter(d => d.value > 0)
    ...
    <div className="space-y-6 animate-fade-in">
          ...
          <div key={key} className={`card p-4 ${bg}`}>
            ...
            {stats.total > 0 && (
              <div className="text-xs text-dark-500 mt-0.5">
                {Math.round((stats[key as keyof typeof stats] / stats.total) * 100)}%
              </div>
            )}
          </div>
        ))}
      ...
      <div className="card p-5">
        ...
        <div className="space-y-2">
          {urgentes.length === 0 && (
            <p className="text-dark-500 text-sm text-center py-6">Sin folios activos</p>
          )}
          {urgentes.map(f => {
            const secs = Math.floor((new Date(f.fecha_vencimiento).getTime() - Date.now()) / 1000)
            const isVencido = f.estatus === 'Vencido' || secs <= 0
            const borderColor = isVencido ? 'border-red-500/30' : secs < 21600 ? 'border-orange-500/20' : 'border-dark-600'
            return (
              <div key={f.id} className={`flex items-center gap-3 px-4 py-3 rounded-lg border ${borderColor} bg-dark-900/50`}>
                <div className={`w-1.5 h-10 rounded-full flex-shrink-0 ${isVencido ? 'bg-red-500' : f.prioridad === 'ALTA' ? 'bg-orange-400' : f.prioridad === 'MEDIA' ? 'bg-yellow-400' : 'bg-blue-400'}`} />
