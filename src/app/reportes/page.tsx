...
export default function ReportesPage() {
  ...
  const inRange = (f: Folio, range: { start: Date; end: Date }) =>
    isWithinInterval(new Date(f.fecha_importacion || f.created_at), range)
  ...
  const weeklyData = Array.from({ length: 8 }, (_, i) => {
    ...
    const abiertos = folios.filter(f => isWithinInterval(new Date(f.fecha_importacion || f.created_at), { start: weekStart, end: weekEnd })).length
