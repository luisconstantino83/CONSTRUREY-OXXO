  ...
  const calc = useCallback((): CountdownResult => {
    if (!fechaVencimiento || estatus === 'Cerrado') {
      return {
        ...
        isExpired: false, formattedTime: 'Cerrado',
        colorState: 'green', colorClass: 'text-dark-400', bgClass: 'bg-dark-700/30'
      }
    }
    ...
    if (expired) {
      ...
      const rh = h % 24
      formattedTime = d + 'd ' + rh + 'h ' + pm + 'm'
    } else {
      formattedTime = ph + 'h ' + pm + 'm ' + ps + 's'
    }
    const colorState = getColor(total, expired)
    return {
      hours: h, minutes: m, seconds: s, totalSeconds: total,
      isExpired: expired, formattedTime,
      colorState, colorClass: COLORS[colorState].text, bgClass: COLORS[colorState].bg
    }
